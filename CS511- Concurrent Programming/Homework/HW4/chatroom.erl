...
do_register(State, Ref, ClientPID, ClientNick) ->
    ...
    Temp = maps:put(ClientPID, ClientNick, State#chat_st.registrations),
    TempNewS = State#chat_st{registrations = Temp},
    TempNewS.

%% This function should unregister a client from this chatroom
do_unregister(State, ClientPID) ->
    Temp = maps:remove(ClientPID, State#chat_st.registrations),
    TempNewS = State#chat_st{registrations = Temp},
    TempNewS.

%% This function should update the nickname of specified client.
do_update_nick(State, ClientPID, NewNick) ->
    Temp = maps:put(ClientPID, NewNick, State#chat_st.registrations),
    TempNewS = State#chat_st{registrations = Temp},
    TempNewS.
...
do_propegate_message(State, Ref, ClientPID, Message) ->
    ...
    maps:map(fun(H, _) ->
        H!{request, self(), Ref, {incoming_msg, ClientNick, State#chat_st.name, Message}}
    end, Clients),
    TempHelp = State#chat_st.history ++ [{ClientNick, Message}],
    TempNewS = State#chat_st{history = TempHelp},
    TempNewS.
