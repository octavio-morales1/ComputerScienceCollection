...
do_join(State, Ref, ChatName) ->
    TempRoomList = State#cl_st.con_ch,
    Temp = whereis(server),
    case maps:is_key(ChatName, TempRoomList) of
        false -> 
            Temp!{self(), Ref, join, ChatName},
            receive
                {ChatID, Ref, connect, History} ->
                    NewState = State#cl_st{con_ch = maps:put(ChatName, ChatID, TempRoomList)},
                    {History, NewState}
            end;
        true -> 
            {err, State}
...
do_leave(State, Ref, ChatName) ->
    ...
    case maps:is_key(ChatName, TempRoomList) of
        false -> 
            {err, State};
        true -> 
            Temp!{self(), Ref, leave, ChatName},
            receive
                {Temp, Ref, ack_leave} ->
                    NewState = State#cl_st{con_ch = maps:remove(ChatName, TempRoomList)},
                    {ok, NewState}
            end
    end.
...
do_new_nick(State, Ref, NewNick) ->
    case NewNick /= State#cl_st.nick of
        false -> {err_same, State}; 
        true ->
			S = whereis(server),
            S!{self(), Ref, nick, NewNick},
            receive
                {S, Ref, ok_nick} ->
                    State1 = State#cl_st{nick = NewNick},
                    {ok_nick, State1};
                {S, Ref, err_nick_used} ->
                    {err_nick_used, State}
            end
...
do_msg_send(State, Ref, ChatName, Message) ->
    TempRoom = maps:get(ChatName, State#cl_st.con_ch),
    TempRoom!{self(), Ref, message, Message},
    receive
        {TempRoom, Ref, ack_msg} ->
